import { Controller, Get, Post, Body, Param, Delete, UseGuards, Query, Req, Patch, ParseArrayPipe } from '@nestjs/common';
...
export class EntriesController {
  ...
  upsertBatch(
    @Req() req: any,
    // Queued client payloads also carry householdId/createdBy; unknown keys are stripped (never trusted).
    @Body(new ParseArrayPipe({ items: CreateEntryDto, whitelist: true, forbidNonWhitelisted: false }))
